...
#endif
	unsigned long (*get_unmapped_area) (unsigned long, unsigned long,
...
/* 
   對應上面的v4l2_file_operations 結構有幾點要注意： 
1. get_unmapped_area，這個最終會assign給file_operations對應的函式。 
   這是系統sys_mmap2使用的函式，會判?虛擬?存空間中是否有足 
   夠的空間工作。 
2. read/write讀流(streaming)
   使用read和write函式，每一個frame都要通?I/O操作，在用戶和內核 
   空間之間copy data。設備的cap結構，要記得設定V4L2_CAP_READWRITE此flag。 
3. ioctl和unlocked_ioctl 
   這兩個都通過file_operations中的unlocked_ioctl調用，v4l2的 
   unlocked_ioctl會優先調用，ioctl可以理解成是序列化的ioctl,
   會上鎖等待前一個ioctl完成，如果在驅動睡眠下會很糟糕。 
4. ioctl讀流(streaming) 
   在設備的cap結構上要記得設定V4L2_CAP_STREAMING此flag，讓用戶與內核空間之間 
   交換buffer pointer，這些buffers將會被映射到用?地址空間，使frame的零拷貝成為可能。 
   “用戶與內核空間之間交換buffer pointer”我們要理解的是這些buffer是 
   在用戶空間還是內核空間。對於內核空間我們通過 mmap()方法映射。 
   對於用戶空間就不需要mmap()，此方法比較難用。 
*/ 
